...
public class ColaRedis : IColaRedisCache, IDisposable
{
    ...
    public ColaRedis(CacheConfigOption options)
    {
        if (options.CacheType == CacheType.Redis.ToInt() || options.CacheType == CacheType.Hybrid.ToInt())
        {
            ...
            #endregion
            SlavesReadRedis ??= MastersWriteRedis!;
        }
    }
    ...
    public async void RedisLockMethodAsync(string key, TimeSpan expiry, Action<ConnectionMultiplexer, ConnectionMultiplexer, ConnectionMultiplexer> success, Action<ConnectionMultiplexer, ConnectionMultiplexer, ConnectionMultiplexer, System.Exception> fail, int database = 0)
    {
