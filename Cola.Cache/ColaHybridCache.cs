...
public class ColaHybridCache : IColaHybridCache
{
    ...
    public static ColaHybridCache Create(IServiceProvider serviceProvider)
    {
        ...
        if (colaRedis == null) throw new System.Exception("未注入 IColaRedisCache 类型");
        var colaMemoryCache = serviceProvider.GetService<IColaMemoryCache>();
        if (colaMemoryCache == null) throw new System.Exception("未注入 IColaMemoryCache 类型");
    ...
    public void Refresh(string key, TimeSpan? expiry = null, int database = 0)
    {
        ...
        if (memoryResult == null)
            ...
            ColaMemoryCache.Set(key, memoryResult, expiry);
        ...
        {
            if (!memoryExists)
                ColaMemoryCache.Set(key, redisResult, expiry);
    ...
    public void RedisLockMethodAsync(string key, TimeSpan expiry, Action<ConnectionMultiplexer, ConnectionMultiplexer, ConnectionMultiplexer> success, Action<ConnectionMultiplexer, ConnectionMultiplexer, ConnectionMultiplexer, System.Exception> fail, int database = 0)
    {
    ...
    public void Subscribe(RedisChannel channel, Action<string, string> handler)
    {
